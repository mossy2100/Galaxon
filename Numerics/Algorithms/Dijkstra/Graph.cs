...
public class Graph
{
    ...
    /// </summary>
    /// <param name="label">The label of the node to be added.</param>
    /// <returns>The newly added node.</returns>
    /// <exception cref="ArgumentException">Thrown when a node with the provided label already exists in
    /// the graph.</exception>
    ...
    /// <param name="bidirectional">A boolean indicating whether the edge is bidirectional. Default is
    /// false.</param>
    /// <returns>The newly added edge.</returns>
    /// <exception cref="ArgumentException">Thrown when a node with the provided label is not found in the
    /// graph.</exception>
    public Edge AddEdge(string node1, string node2, double distance, bool bidirectional = false)
    {
        ...
        // Add the edge.
        Edge newEdge = new (n1, n2, distance, bidirectional);
        edges.Add(newEdge);
    ...
    /// <param name="endNodeLabel">The label of the end node. If null, the method will find the shortest
    /// path to all nodes.</param>
    /// <exception cref="ArgumentException">Thrown when the start node or end node is not found in the
    /// graph.</exception>
    public void ShortestPath(string startNodeLabel, string? endNodeLabel = null)
    {
        ...
        Node? endNode = endNodeLabel == null
            ? null
            : GetNode(endNodeLabel) ?? throw new ArgumentException("End node not found.");
        ...
        while (true)
        {
            ...
            IEnumerable<Edge> edgesToNeighbour = edges.Where(e =>
                e.node1 == currentNode || (e.bidirectional && e.node2 == currentNode));
