...
public class TestJulianDates
{
    ...
    public void TestDateOnlyToJulianDay()
    {
        ...
        Assert.AreEqual(JulianDateUtility.DateOnly_to_JulianDate_UT(date), 1721425.5);

        // Test current date.
        date = new DateOnly(2022, 6, 8);
        Assert.AreEqual(JulianDateUtility.DateOnly_to_JulianDate_UT(date), 2459738.5);

        // Test middle of range.
        date = new DateOnly(5000, 7, 2);
        Assert.AreEqual(JulianDateUtility.DateOnly_to_JulianDate_UT(date), 3547454.5);

        // Test end of range.
        date = new DateOnly(9999, 12, 31);
        Assert.AreEqual(JulianDateUtility.DateOnly_to_JulianDate_UT(date), 5373483.5);
    }
    ...
    public void TestDateOnlyFromJulianDay()
    {
        ...
        date2 = JulianDateUtility.JulianDate_UT_to_DateOnly(1721425.5);
        Assert.AreEqual(date1.GetTicks(), date2.GetTicks());

        // Test current date.
        date1 = new DateOnly(2022, 6, 8);
        date2 = JulianDateUtility.JulianDate_UT_to_DateOnly(2459738.5);
        Assert.AreEqual(date1.GetTicks(), date2.GetTicks());

        // Test middle of range.
        date1 = new DateOnly(5000, 7, 2);
        date2 = JulianDateUtility.JulianDate_UT_to_DateOnly(3547454.5);
        Assert.AreEqual(date1.GetTicks(), date2.GetTicks());

        // Test end of range.
        date1 = new DateOnly(9999, 12, 31);
        date2 = JulianDateUtility.JulianDate_UT_to_DateOnly(5373483.5);
        Assert.AreEqual(date1.GetTicks(), date2.GetTicks());
    }
