...
public class Vsop87ImportService(AstroDbContext astroDbContext)
{
    ...
    ///
    /// As per AA2, we're using VSOP87D data files, which contain values for heliocentric dynamical
    /// ecliptic and equinox of the date.
    /// </summary>
    /// <seealso href="https://www.caglow.com/info/compute/vsop87"/>
    public async Task Import()
    {
        for (int planetNum = 1; planetNum <= 8; planetNum++)
        {
            string planetName = AstroObjectRepository.PLANET_NAMES[planetNum]!;
            Console.WriteLine();
            ...
            while (await sr.ReadLineAsync() is { } line)
            {
                ...
                {
                    Console.WriteLine(
                        "Could not read planet number (code of body), skipping line.");
                    continue;
                }
